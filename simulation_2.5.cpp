...
void simulation(bool affiche, int nbp, int rank, char nom_du_groupe, MPI_Comm New_Comm)
{
    const int tag = 1;
    ...
    if (nom_du_groupe == 'B') {
        ...
        size_t division = contexte.taux_population / nbp;

        size_t population_proc; // nombre d'individus par proc
        if (rank == nbp - 1) {
            population_proc = contexte.taux_population - division * (nbp - 1);
        }
        else {
            population_proc = division;
        }

        std::vector<épidémie::Individu> population;
        population.reserve(population_proc);
        ...
        graine_aléatoire += (rank - 1) * division; // pour traverser les graines comme en séquentiel
        for (std::size_t i = 0; i < population_proc; ++i) // on peut paralléliser cette boucle, mais je n'arrive pas à obtenir des mêmes résultats ... 
        {
        ...
        int division_grippe = (contexte.taux_population * 23) / 100; // # imunisés

        // pour le faire fonctionner même s'il n'est pas divisible par 100 [juste comme au début avec découpage des individus]
        int nombre_immunisés_grippe; 
        if (rank == nbp - 1) {
            nombre_immunisés_grippe = division_grippe - (nbp - 1) * (population_proc * 23) / 100;
        }
        else {
            nombre_immunisés_grippe = (population_proc * 23) / 100;
        }
        ...
        while (!quitting)
        {
            ...
            //std::cout << "TestTestTestTest" << std::endl << std::flush;
            if (jours_écoulés % 365 == 0)// Si le premier Octobre (début de l'année pour l'épidémie ;-) )
            {
                ...
                for (int ipersonne = nombre_immunisés_grippe; ipersonne < int(population_proc); ++ipersonne)
                {
            ...
            {
                for (int ipersonne = nombre_immunisés_grippe; ipersonne < nombre_immunisés_grippe + n_contaminations; ++ipersonne)
                {
            ...
            auto& stat_grille_loc = grille.getStatistiques();
            auto stat_grille_reduce = grille.getStatistiques(); // juste pour copier les valeurs ici

            MPI_Allreduce(stat_grille_loc.data(), stat_grille_reduce.data(), stat_grille_loc.size() * 3, MPI_INT, MPI_SUM, New_Comm);
            grille.setStatistiques(stat_grille_reduce); // maj grille
...
int main(int argc, char* argv[])
{
    ...
    MPI_Comm_split(MPI_COMM_WORLD, colour, rank, &New_Comm); // groupe A - rank 0; groupe B - ranks 1, 2, 3...

    //std::cout << nom_du_groupe << " Mon rank est  " <<  rank << ' ';
    ...
    sdl2::init(argc, argv);
    {
        simulation(affiche, nbp, rank, nom_du_groupe, New_Comm);
    }
    sdl2::finalize();
