...
void simulation(bool affiche)
{
    ...
    std::size_t jours_écoulés_glob = 0;
    ...
    while (!quitting)
    {
        ...
        jours_écoulés += 1;
        jours_écoulés_glob = jours_écoulés;
    }// Fin boucle temporelle
    output.close(); // if rank == 1 par ex.

    std::cout << "Temps de calcul de la simulation par jour : " << (delta_sim).count() / jours_écoulés_glob << "s" << std::endl; // if rank == 0

    std::cout << "Temps de calcul de l'affichage par jour : " << (delta_aff).count() / jours_écoulés_glob << "s" << std::endl; // if rank == 1

    auto end_glob = std::chrono::system_clock::now();
    std::chrono::duration<double> delta_glob = end_glob - start_glob; // if rank == 0
    std::cout << "Temps de calcul global par jour : " << (delta_glob).count() / jours_écoulés_glob << "s" << std::endl; // if rank == 0
}
